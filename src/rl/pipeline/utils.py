__all__ = ["setup_from_cfg", "DEFAULT_TRAINER_KWARGS", "get_stage", "SUPPORTED_STAGES"]
...
from rl.callbacks import HydraConfigCallback
from rl.utils import wrong_type_err_msg
...
def setup_from_cfg(
    ...
    if cfg.ckpt_path is not None:
        model_class: ty.Type[pl.LightningModule] = hydra.utils.get_class(cfg.model._target_)
    ...
    if setup_trainer:
        ...
        # add hydra config logger callback
        hydra_cb = HydraConfigCallback(cfg)
        pl_callbacks.append(hydra_cb)
...
def _replace_true_w_on(cfg: DictConfig) -> None:
    """This fixes an OmegaConf bug, where if any key is 'on', then it is replaced by the literal True..."""
...
def _get_cb(cfg: DictConfig) -> ty.List[pl.Callback]:
    ...
    if isinstance(cb, (OmegaConf, ListConfig)):
        ...
        raise ValueError(
            wrong_type_err_msg(
                cb,
                (pl.Callback, OmegaConf, ListConfig),
